	...
	<div id = "table_container">
		<?php 
			...
			{ 
				...
				$ret_date = $_POST['ReturnDate'];
				//echo $Place;
				$date_1 = new DateTime($dep_date);
				$dep_date = $date_1->format('Y-m-d');
				$date_2 = new DateTime($ret_date);
				$ret_date = $date_2->format('Y-m-d');
				//echo $Check_In;
				$adult = $_POST['Adult'];
				...
				$var = "SELECT *FROM flights WHERE from_ = '$l_from' AND to_ = '$g_to' AND dep_date = '$dep_date' AND arrival_date = '$ret_date' AND seats >= '$person'";
				$res = mysql_query($var);
				if(mysql_num_rows($res) > 0)
				{
					...
					while($rows = mysql_fetch_array($res))
					{
						...
						echo '<td><a href = "test.php"><button>Book Now</button></a></td>';
						echo '</tr>';
					}
					echo '</table>';
				}
